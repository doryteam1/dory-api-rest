...
async function getTipo(page = 1, tipo){
  const offset = helper.getOffset(page, config.listPerPage);
  let cad= tipo;
...
async function getArticulos(page = 1, cadena){
  const offset = helper.getOffset(page, config.listPerPage);
  let cad= '%'+cadena+'%';
...
async function getRevistas(page = 1, cadena){
  const offset = helper.getOffset(page, config.listPerPage);
  let cad= '%'+cadena+'%';
...
async function getNoticias(page = 1, cadena){
  const offset = helper.getOffset(page, config.listPerPage);
  let cad= '%'+cadena+'%';
